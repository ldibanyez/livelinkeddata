...
public class GraphRecomputationInsert {
  ...
  public void init() throws URISyntaxException, FileNotFoundException, IOException{
      ...
      try {
          ...
          Logger.getLogger(GraphRecomputationInsert.class.getName()).log(Level.SEVERE, null, ex);
  ...
  public void prepare() throws URISyntaxException, LoadException {
    ...
    //Execute view before changes 
	  ...
	  //Logger.getLogger(GraphRecomputationRemote.class.getName()).log(Level.INFO, "Target size before = {0}", initialSize);

	// An insert happens at the source
	  numInsertions =  (int)(Math.floor(initialSize*percentage)); 
	  Helper.insEndpoint(endpoint, predicate,numInsertions);
  }

  @TearDown(org.openjdk.jmh.annotations.Level.Iteration)
  public void check(){
      assert(initialSize+numInsertions == target.size());
	  //Logger.getLogger(GraphRecomputationInsert.class.getName())
      //        .log(Level.INFO, "{0} + {1} = {2}", 
      //        new Object[]{initialSize, numInsertions, target.size()});
	  target.clearDefault();
	  target.clearNamed();
  }
